...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "ايقاف مؤقت":
    ...
    elif command == "الغاء كتم التشغيل":
        ...
        check = db.get(chat_id)
        txt = f"» تم تخطي التشغيل بواسطة {mention} !"
        popped = None
        try:
            popped = check.pop(0)
            if popped:
        ...
        if int(command) in [1, 3]:
            if (duration_played - duration_to_skip) <= 10:
                ...
                return await CallbackQuery.answer(
                    f"» البوت غير قادر علي التقدم لان المدة تتجاوز المدة المسموح بها.\n\nالتشغيل حالياً :** {bet}** دقائق من**{duration}** دقيقة.",
                    show_alert=True,
                )
        ...
        await mystic.edit_text(f"{string}\n\nتم التغيير بنجاح بواسطة : {mention} !")
