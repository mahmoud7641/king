...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"اكثر الاشياء التي تم تشغيلها على {MUSIC_BOT_NAME}\n\n**العنوان:** {title}\n\nتم تشغيله** {co} **مرة"
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"[تليجرام ميديا](https://t.me/YY5Y8) ** تم تشغيله {count} مرة**\n\n"
                else:
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                ...
                continue
            limit += 1
            msg += f" `{extract}` تم تشغيله{count} مرة علي بوتك.\n\n"
        temp = (
